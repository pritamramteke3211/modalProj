...
const Login = ({navigation}) => {
  ...
  const login = async () => {
    try {
      ...
      const validatedNumber = await validateNumber.validate(
  ...
  const createProfile = async googleData => {
    try {
      ...
      googleData.user.familyName &&
        formData.append('lastName', googleData.user.familyName);
  ...
  const onPressGoogleLogin = async () => {
    try {
      ...
      if (loginDetail?.firstName) {
        ...
        x = JSON.parse(x);
        dispatch(
          updateAuthUserData({...loginDetail, accessToken: x?.accessToken}),
        );
      } else {
  ...
  const appleLogin = async () => {
    ...
    try {
      ...
      const {email, identityToken, realUserStatus /* etc */} =
    ...
    <WrapperContainer isLoading={isloading}>
      <View style={{padding: 20, flex: 1}}>
        ...
        <Text
          ...
          {strings.to_continue_phone_number}
        </Text>
        <View style={{marginTop: rspH(12)}}>
          <TextInputCustom
            returnKeyType="done"
            keyboardType={'numeric'}
            onChangeText={setmobileNumber}
            placeHolderString={strings.enter_phone_number}
          />
          ...
          <FlexSBContainer
            ...
            <FastImage
              ...
              source={showGif ? imagePath.success : ''}
            />
          </FlexSBContainer>
          ...
          <MainButton
            ...
            btnStyle={{marginTop: rspH(4.4)}}
          />
          <Text style={styles.tappingDetail}>
            {strings.tapping_to_continue}
            <Text
              onPress={() => navigationToPolicy(T_AND_C)}
              style={{fontFamily: fontFamily.bold, color: colors.light_black}}>
              {' Terms of Use'}
        ...
        <View style={styles.socialContainer}>
          {Platform.OS === 'ios' && (
            <SocialButton
              ...
              btnText={strings.continue_with_apple}
            />
          )}
          <SocialButton
            onPress={onPressGoogleLogin}
            icon={imagePath.google}
            btnText={strings.continue_to_google}
          />
          {!userData?.guest && (
