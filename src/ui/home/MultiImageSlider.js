...
const ImageHandler = ({name}) => {
  return <FastImage style={styles.normalImage} resizeMode={Platform.OS === 'ios' ? 'cover' : 'center'} source={{uri: `${FILE_BASE_URL}${name}`}} />;
};
...
const MultiImageSlider = (_) => {
  ...
  let onlyPhotos = [];
  const isVideoPost = data.length > 1 ? '' : data.find((item) => item?.mediaType === 'videos');
  ...
  return (
    <View style={styles.container}>
      {/* <FlatList horizontal initialNumToRender={2} pagingEnabled renderItem={renderMultiImage} data={onlyPhotos} decelerationRate={'fast'} keyExtractor={(_, index) => index.toString()} /> */}
      {/* <View style={{marginTop: moderateScaleVertical(20)}}>{!!isAudioPost && <AudioHandler audio={isAudioPost?.name} outerIndex={outerIndex} />}</View> */}
    </View>
  );
};
