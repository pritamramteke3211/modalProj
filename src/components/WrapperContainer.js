...
const WrapperContainer = ({
  ...
  const displayNotification = async remoteMessage => {
    if (
      ...
      const filter = x.find(
        item => item.id === remoteMessage?.data?.postId?.toString(),
      );
      let hasData = [];
      if (filter) {
  ...
  useEffect(() => {
    const sendMessageListener = DeviceEventEmitter.addListener(
      'sendMessage',
      remoteMessage => {
        setTimeout(() => {
  ...
  return (
    <View
      style={{
        ...
        paddingBottom:
          Platform.OS === 'ios' ? 0 : removeBottomInset ? 0 : insets.bottom,
      }}>
      <FocusAwareStatusBar
